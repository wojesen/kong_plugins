...
local proto_fake_file   = require("kong.plugins.grpc-gateway2.proto").proto_fake_file
local json   = require("kong.plugins.grpc-gateway2.json")
local table   = require("kong.plugins.grpc-gateway2.table")
local corerequest  = require("kong.plugins.grpc-gateway2.core-request")
local pb                = require("pb")
local ngx               = ngx
local string            = string
local table             = table
...
local function get_request_table()
    ...
    then
        local req_body, _ = corerequest.get_body()
        if req_body then
...
function _M.map_message(field, default_values, request_table, real_key)
    ...
    for name, _, field_type in pb.fields(field) do
        ...
        if ty ~= "enum" and field_type:sub(1, 1) == "." then
            ...
            elseif table.isarray(request_table[name]) then
                local sub_array = table.new(#request_table[name], 0)
