...
local grpc_proto  = require("kong.plugins.grpc-gateway2.proto")
--local core   = require("kong.plugins.grpc-gateway2")
local json   = require("kong.plugins.grpc-gateway2.json")
local table   = require("kong.plugins.grpc-gateway2.table")
local coreresponse  = require("kong.plugins.grpc-gateway2.core-response")
local pb     = require("pb")
local ngx    = ngx
local string = string
    ...
    if grpc_status then
        ...
        if status_detail_type and details then
            ...
            for _, detail in ipairs(details) do
                ...
                table.insert(decoded_details, err_or_value)
            end
        ...
        local response, err = json.encode(resp_body)
...
return function(ctx, proto, service, method, pb_option, show_status_in_body, status_detail_type)
    local buffer = coreresponse.hold_body_chunk(ctx)
    if not buffer then
        return nil
    end
    ...
    local response, err = json.encode(decoded)
    if not response then
        err_msg = "failed to json_encode response body"
        ngx.arg[1] = err_msg
