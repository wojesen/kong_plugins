...
local lrucache          = require("kong.plugins.grpc-gateway2.lrucache")
local pcall         = pcall
local ipairs        = ipairs
...
local protos
local lrucache_proto = lrucache.new({
    ttl = 300, count = 100
})
...
local function create_proto_obj(content)
...
function _M.fetch(proto_id,content)
    --return lrucache_proto(proto_id, "1",
    --                      create_proto_obj, content)
    return create_proto_obj(content)
end
