...
local Parser = meta "Parser" do
   ...
   function Parser:parsefile(name)
      ...
      for _, path in ipairs(self.paths) do
         ...
         if fh then
            ...
            --print_r({"hhhhhhhhhhhhhhhhhhhhhhhh"})
            --print_r({fn})
            --print_r({content})
            --print_r({"jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj"})
   ...
   local toplevel = {} do
      ...
      function toplevel:import(lex, info)
         ...
         lex:line_end()
         --print_r({"tttttttttttttttttttttttttttttttttttt"})
         --print_r({name})
         --print_r({"uuuuuuuuuuuuuuuuuuuuuuuuuuuu"})
      ...
      function toplevel:option(lex, info)
         --print_r({"888888888888888888"})
         local ident = lex:option_name()
         --print_r({ident})
         --print_r({"99999999999999999999"})
         lex:expected "="
         local value = lex:constant()
         --print_r({value})
         --print_r({"qqqqqqqqqqqqqqqqqqqqq"})
         lex:line_end()
         --print_r({"wwwwwwwwwwwwwwwwwwww"})
         local options = info and default(info, 'options') or {}
         --print_r({"eeeeeeeeeeeeeeeeeeeeeee"})
         options[ident] = value
         return options, self
      end
   ...
   function Parser:parse(src, name)
      ...
      while not lex:eof() do
         ...
         --print_r({"3333333333333333333333"})
         --print_r({ident})
         local top_parser = toplevel[ident]
         --print_r({"4444444444444444444"})
         --print_r(toplevel)
         if top_parser then
            --print_r({"5555555555555555"})
            top_parser(ctx, lex, info)
            --print_r({"666666666666666666"})
         else
            lex:error("unknown keyword '"..ident.."'")
         end
         lex:line_end "opt"
      end
      --print_r({"77777777777777777777777"})
