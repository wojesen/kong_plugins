...
local function init()
  ...
  if kong_subsystem == "http" then
    metrics.status = prometheus:counter("http_status",
                                        ...
                                        {"service", "route", "code", "consumer","credential",
                                         "envId","envName","groupId","groupName","apiId","apiName","namespace","model",
  ...
  metrics.latency = prometheus:histogram("latency",
                                         ...
                                         {"service", "route", "type", "consumer","credential",
                                          "envId","envName","groupId","groupName","apiId","apiName","namespace","model",
                                          "userAgent","remoteAddr"},
  ...
  metrics.total_tokens = prometheus:counter("total_tokens",
          "Total total_tokens in token " ..
                  "consumed per service/route in Kong",
          {"service", "route", "type", "consumer","credential",
           "envId","envName","groupId","groupName","apiId","apiName","namespace","model",
           "userAgent","remoteAddr"})
  metrics.prompt_tokens = prometheus:counter("prompt_tokens",
          "Total prompt_tokens in token " ..
                  "consumed per service/route in Kong",
          {"service", "route", "type", "consumer","credential",
           "envId","envName","groupId","groupName","apiId","apiName","namespace","model",
           "userAgent","remoteAddr"})
  metrics.completion_tokens = prometheus:counter("completion_tokens",
          "Total completion_tokens in token " ..
                  "consumed per service/route in Kong",
          {"service", "route", "type", "consumer","credential",
           "envId","envName","groupId","groupName","apiId","apiName","namespace","model",
           "userAgent","remoteAddr"})
...
if kong_subsystem == "http" then
  function log(message, serialized)
    ...
    local credential
    if message and message.request and message.request.headers  and message.request.headers["x-credential-identifier"] then
      credential = message.request.headers["x-credential-identifier"]
    else
      credential = ""
    end
    ...
    labels_table[4] = consumer_name
    labels_table[5] = credential
    if serialized.envId then
      labels_table[6] = serialized.envId
    else
      labels_table[6] = ""
    end
    if serialized.envName then
      labels_table[7] = serialized.envName
    else
      labels_table[7] = ""
    end
    if serialized.groupId then
      labels_table[8] = serialized.groupId
    else
      labels_table[8] = ""
    end
    if serialized.groupName then
      labels_table[9] = serialized.groupName
    else
      labels_table[9] = ""
    end
    if serialized.apiId then
      labels_table[10] = serialized.apiId
    else
      labels_table[10] = ""
    end
    if serialized.apiName then
      labels_table[11] = serialized.apiName
    else
      labels_table[11] = ""
    end
    if serialized.namespace then
      labels_table[12] = serialized.namespace
    else
      labels_table[12] = ""
    end
    if serialized.model then
      labels_table[13] = serialized.model
    else
      labels_table[13] = ""
    end
    if serialized.userAgent then
      labels_table[14] = serialized.userAgent
    else
      labels_table[14] = ""
    end
    if serialized.remoteAddr then
      labels_table[15] = serialized.remoteAddr
    else
      labels_table[15] = ""
    end
    ...
    if serialized.total_tokens then
      --labels_table[6] = serialized.envId
      --labels_table[7] = serialized.envName
      --labels_table[8] = serialized.groupId
      --labels_table[9] = serialized.groupName
      ...
      --labels_table[11] = serialized.apiName
      --labels_table[12] = serialized.namespace
      metrics.total_tokens:inc(serialized.total_tokens, labels_table)
    end
    if serialized.prompt_tokens then
      --labels_table[6] = serialized.envId
      --labels_table[7] = serialized.envName
      --labels_table[8] = serialized.groupId
      --labels_table[9] = serialized.groupName
      ...
      --labels_table[11] = serialized.apiName
      --labels_table[12] = serialized.namespace
      metrics.prompt_tokens:inc(serialized.prompt_tokens, labels_table)
    end
    if serialized.completion_tokens then
      --labels_table[6] = serialized.envId
      --labels_table[7] = serialized.envName
      --labels_table[8] = serialized.groupId
      --labels_table[9] = serialized.groupName
      ...
      --labels_table[11] = serialized.apiName
      --labels_table[12] = serialized.namespace
      metrics.completion_tokens:inc(serialized.completion_tokens, labels_table)
    end
    if serialized.consumer ~= nil then
      ...
      labels_table4[4] = serialized.consumer.username
