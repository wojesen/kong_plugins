local policy_cluster = require "kong.plugins.token-limiting.policies.cluster"
...
local get_local_key = function(conf, identifier, period, period_date)
  ...
  if conf.total and conf.total >0 then
    return fmt("tokenlimit:%s:%s:%s", route_id, service_id, identifier)
  end
  return fmt("tokenlimit:%s:%s:%s:%s:%s", route_id, service_id, identifier,
             period_date, period)
end
...
return {
  ...
  ["redis"] = {
    increment = function(conf, limits, identifier, current_timestamp, value)
      ...
      local periods = timestamp.get_timestamps(current_timestamp)
      if conf.total and conf.total>0 then
        local cache_key = get_local_key(conf, identifier, "total", "")
