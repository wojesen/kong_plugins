        ...
        if valid_method[http_method] then
          ...
          if err then
            ...
            table.insert(info[http_method], {
              ...
              pb_state = parsed.pb_state,
              pbstate = parsed.pb_state
            })
          end
        end
      end
...
function deco:upstream(body)
  ...
  print_r({"kkkkkkkkkkkkkkkkkkkkkkkkkkkkk"})
  print_r({self.endpoint.input_type})
  local pb_old_state = pb.state(pbstate)

  --local map_message = util.map_message(m.input_type, default_values or {})
  local ok, encoded = pcall(pb.encode, self.endpoint.input_type, payload)
  print_r({ok})
  print_r({encoded})
  pb.state(pb_old_state)
  print_r({"lllllllllllllllllllllllllllll"})
